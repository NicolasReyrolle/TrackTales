...
class WorkoutManagerSeasonalAggregationsMixin:
    """Seasonal pattern aggregation helpers for WorkoutManager."""
    ...
    def _aggregate_by_seasonal_unit(
        ...
        if activity_type != "All" and "activityType" not in self.workouts.columns:
            return {}
    ...
    def _aggregate_by_year(
        ...
        """Aggregate a metric per calendar year.

        Groups workouts by the year of ``startDate`` and returns the total metric
        value for each observed year.  Only years with at least one matching
        workout appear in the result.
        ...
        year_series = workouts["startDate"].dt.year
        grouped: pd.Series = aggregation(workouts.groupby(year_series)[column])
        transformed = transformation(grouped)
        return {str(year): float(v) for year, v in transformed.items()}
    ...
    def get_count_by_year(
        ...
        """Return total workout count for each observed calendar year."""
        return self._aggregate_by_year(
            "activityType",
            lambda x: x.count().astype(float),
            lambda x: x,
            column_check="activityType",
    ...
    def get_distance_by_year(
        ...
        """Return total distance for each observed calendar year in the specified unit."""
        return self._aggregate_by_year(
            "distance",
            lambda x: x.sum(),
            lambda x: x.div(self._get_length_unit_divisor(unit)),
    ...
    def get_duration_by_year(
        self,
        activity_type: str = "All",
        start_date: datetime | pd.Timestamp | None = None,
        end_date: datetime | pd.Timestamp | None = None,
    ) -> dict[str, float]:
        """Return total duration in hours for each observed calendar year."""
        return self._aggregate_by_year(
            "duration",
            lambda x: x.sum(),
            lambda x: x.div(3600),
    ...
    def get_calories_by_year(
        ...
        """Return total calories burned for each observed calendar year."""
        return self._aggregate_by_year(
            "sumActiveEnergyBurned",
            lambda x: x.sum(),
            lambda x: x,
