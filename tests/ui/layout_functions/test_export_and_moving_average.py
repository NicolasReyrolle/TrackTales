...
class TestExportHandlers:
    ...
    def test_handle_markdown_export_calls_export_and_download(self, monkeypatch) -> None:
        # Create the mock for workouts
        workouts_mock = MagicMock()
        workouts_mock.export_to_markdown.return_value = "# report"

        # Safely modify the global state using monkeypatch
        monkeypatch.setattr(state, "workouts", workouts_mock)
        monkeypatch.setattr(state, "selected_activity_type", "Running")
        monkeypatch.setattr(state, "date_range_text", "2024-03-01 - 2024-03-31")

        # Mock the download function
        download_mock = MagicMock()
        monkeypatch.setattr("ui.layout.ui.download", download_mock)
